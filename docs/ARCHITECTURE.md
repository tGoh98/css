...
| Penpot | GitHub releases only | daily | Low volume; OSS so GH releases are signal. **penpot.app publishes no blog feed** — the `Penpot blog` source row is disabled (2026-08-06), see decision log |
...
| 2026-07-01 | Digest worker: fail fast on a logged-out CLI + actionable alert | Both the 09:00 daily (`day` 2026-06-30) and monthly (`month` 2026-06-01) digests failed because the local Claude Code CLI was logged out — `claude --print` returned `Not logged in · Please run /login`. The generic retry treated it as transient, burning the full 60s/300s/1500s back-off (~30 min) per period on a wall that never moves, and the failure email showed only a generic exit-1 message. Two fixes: (1) `isAuthError()` detects the logged-out signature; `invokeClaude` passes it to `withRetry`'s new `shouldRetry` predicate so auth failures abort immediately (and `invokeClaudeOnce` skips the pointless text-fallback invocation), and (2) `notifyDigestFailure()` mirrors the same regex to lead the subject + body with "CLI logged out — run `/login` on the Mac" when that's the cause. Recovery is unchanged: re-authenticate, then re-run the named failed range(s). |
| 2026-07-25 | Retire the Seeking Alpha (FIG) source — do NOT re-add | **Don't re-investigate this as a "dead source" — it's dead on purpose.** History: the SA poller lived in `analyst.ts` and was removed as unintended collateral on 2026-05-13 (commit e6f0dad) when the Finnhub-backed Analyst *tab* was dropped — tab and ingest shared a filename but not a purpose. That left source row #7 `enabled` in Neon with no code behind it, which `ingest-health.ts` surfaced as 71 days stale on 2026-07-24. We evaluated reviving it (feed is alive + free; built + tested a `src/ingest/seeking-alpha.ts` poller that ingested 18 items cleanly) and then deliberately **retired it instead**, for two reasons: (1) **compliance** — SA's feed `<description>` restricts use to personal/non-commercial and prohibits public republishing, but the content site is public since 2026-05-14 (only `/admin/*` is gated), so `/feed` would republish verbatim SA titles/snippets to anyone; unlike Google News/SEC/HN/Reddit, SA states this restriction explicitly in the feed payload. (2) **marginal value** — SA's notable+ signal ratio (52%) merely matches Google News (51%), which already covers FIG financial headlines. Poller, `/api/cron/ingest/seeking-alpha` route, and hourly `cron.yml` matrix entry removed; source #7 + its 47 items (29 historical + 18 from the revival test) deleted from Neon (`onDelete: cascade`). If analyst commentary is ever wanted again, it must ship with public-display gating (admin-only render, or digest-synthesis-only — no verbatim public reproduction). |
| 2026-08-06 | Digest worker: JSON retries before the text fallback; warm-up is bounded and non-fatal | Audit after the Figma Q2 earnings run surfaced three things in `run-digest.ts`. (1) **Silent permanent downgrade.** `invokeClaudeOnce` fell into the plain-text path on *any* JSON failure. The real failure was `API Error: Connection closed mid-response` — a transient streaming blip, not a capability gap — so every digest from 2026-08-05 on was recorded as `text fallback` while burning a second full CLI invocation immediately, before `withRetry` could back off. Now `invokeClaudeOnce` takes `allowTextFallback` and re-throws on non-final attempts, so JSON is retried at 60s/300s and only the last attempt may degrade; a genuinely JSON-less CLI still yields a digest. (2) **Unbounded warm-up.** `warmUpDb` claimed a "~40 s" budget but allowed 6 attempts at base 500 ms × factor 4 = 170 s of sleep alone, plus per-attempt connect hangs; the 2026-08-06 run spent **606 s** there before doing any work. Now 5 attempts × factor 3 (~20 s of back-off) under a hard `WARM_UP_BUDGET_MS` = 60 s deadline; measured 475 ms after the fix. (3) **Warm-up failure was fatal.** It called `process.exit(1)`, discarding the whole run over a slow wake that the per-range retry budgets would have absorbed — now logged at `warn` and execution continues. |
| 2026-08-06 | Digest log: one sink, not two | Every line in `~/.local/state/css/digest.log` was written twice — `log()` appends to the file itself *and* the launchd plists redirected stdout into the same file (`>> digest.log 2>&1`). ~450 KB of the 904 KB log was duplicate, and any `grep -c` over it read 2× high. Redirect removed from all three plists (repo copies + installed); `log()` remains the single writer, and `StandardOutPath`/`StandardErrorPath` now usefully capture crashes that bypass the logger (tsx resolution errors, stack traces) instead of sitting at 0 bytes. `db_warm` events are tagged with `period` since all three agents share the log and fire together on `RunAtLoad`. |
| 2026-08-06 | Fix `Sketch blog`, disable `Penpot blog` — both had never ingested anything | `ingest-health.ts` showed both with `last_polled_at IS NULL` since creation. Both rows had been seeded speculatively — their `config_json` carried `"note": "URL unverified — may 404"` — and both URLs did 404. The poller records per-run failures into `result.errors`, but nothing escalates a source that has *never* succeeded, so they failed silently for months. Sketch's feed exists at `https://www.sketch.com/blog/feed.xml` (the row had `/blog/feed/`, which 404s); fixed and verified — the repair run inserted 8 items with zero errors. Penpot publishes **no** blog feed (`/blog/feed.xml`, `/blog/rss.xml`, `/atom.xml`, `/feed.xml`, `/blog/index.xml` all 404), so that row is disabled rather than left failing forever; Penpot product news is already covered by the working `Penpot GitHub releases` source. A Discourse feed exists at `community.penpot.app/latest.rss` but is forum chatter, not the blog, so it was deliberately not substituted. Repair is scripted and idempotent: `scripts/fix-competitor-feeds.ts`. **Follow-up worth doing:** `ingest-health.ts` should flag never-succeeded sources distinctly from stale ones. |
| 2026-08-06 | Rename `tick-15m` / `tick-hourly` → `tick-fast` / `tick-slow` | The route names asserted a cadence no caller ever used. GitHub Actions drives the real cadence by hitting the per-source `ingest/*` routes directly (`*/5` and hourly); these two fan-out routes are invoked **only** by Vercel Cron, once a day each (`0 9` / `0 10`), because Hobby caps cron frequency at daily and 2 jobs/project. So `tick-15m` ran daily and nothing ran every 15 minutes. Renamed to describe the source tier rather than a schedule, with the constraint documented in both route files. `notifyIngestSaturation` labels updated to match. |

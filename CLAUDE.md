...
- Scheduled digests (Opus 4.7) run **locally** via `claude --print` on the owner's Mac (Max plan capacity), writing `summary_md` back to Neon — quality matters more than latency here and Max-plan capacity costs the same either way (see `CLAUDE_MODEL` in `scripts/run-digest.ts`)
